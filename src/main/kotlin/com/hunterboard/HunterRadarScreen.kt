...
class HunterRadarScreen(
    ...
    override fun render(context: DrawContext, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        context.fill(pX, pY, pX + PANEL_W, pY + panelH, 0xF0101020.toInt())
        fillBorderSolid(context, pX, pY, PANEL_W, panelH, ModConfig.accentColor(), 2)

        // ── Header ──────────────────────────────────────────────────────────
        val title = "${slots.size} apparition${if (slots.size > 1) "s" else ""} possible${if (slots.size > 1) "s" else ""}"
        context.drawText(textRenderer, title, pX + PADDING, pY + 7, 0xFFFFFFFF.toInt(), true)
        ...
        // Separator
        context.fill(pX + 2, pY + HEADER_H - 1, pX + PANEL_W - 2, pY + HEADER_H, 0xFF333355.toInt())
        ...
        for (dispIdx in displayIndices.indices) {
            ...
            context.drawText(textRenderer, pctStr,
                cx + (CELL_W - tw) / 2, cy + 3,
                slotColors.getOrElse(idx) { 0xFFFFFFFF.toInt() }, false)
        ...
        if (maxScroll > 0) {
            ...
            context.fill(sbX, thumbY, sbX + SB_W, thumbY + thumbH,
                if (sbHov || sbDragging) ModConfig.accentColor() else 0xFF445566.toInt())
        }
    ...
    private fun resolveRarityColor(species: Species): Int {
        ...
        return runCatching {
            ...
            if (all.isEmpty()) return 0xFFFFFFFF.toInt()
            val matching = if (biomeId == null) all
                           else all.filter { entryMatchesBiome(it, biomeId, biomeEntry) }.ifEmpty { all }
            entryBucketColor(matching.maxByOrNull { entryBucketWeight(it) } ?: return 0xFFFFFFFF.toInt())
    ...
    private fun entryBucketColor(entry: Any): Int {
        return runCatching {
            val bucket = entry.javaClass.getMethod("getBucket").invoke(entry) ?: return 0xFFFFFFFF.toInt()
            val name = (bucket.javaClass.getMethod("getName").invoke(bucket) as? String)
                ?.lowercase()?.replace("-", "_") ?: return 0xFFFFFFFF.toInt()
            when (name) {
    ...
    private fun drawLegend(context: DrawContext, belowY: Int) {
        ...
        for ((i, e) in entries.withIndex()) {
            ...
            context.drawText(tr, label, curX + sqSz + gap, legendY, 0xFFCCCCCC.toInt(), false)
            curX += widths[i] + gap
        }
    }
    ...
    private fun drawTooltip(ctx: DrawContext, species: Species, mx: Int, my: Int) {
        ...
        try {
            ...
            if (typeLine.isNotBlank()) lines += Line(typeLine, 0xFFCCCCCC.toInt())
        } catch (_: Exception) {}
    ...
    private fun drawSortFilterButtons(context: DrawContext, mx: Int, my: Int, pX: Int) {
        ...
        context.drawText(textRenderer, "Filtre", btnX + (BTN_W - textRenderer.getWidth("Filtre")) / 2,
            btnY - textRenderer.fontHeight - 2, 0xFF888888.toInt(), false)

        // Filter button — EV color when active
        val filterKey = RadarEnhancer.EV_ORDER.getOrNull(evFilter)
        val filterEvColor = if (filterKey != null) RadarEnhancer.EV_COLORS[filterKey] ?: 0xFFCCCCCC.toInt() else 0xFFCCCCCC.toInt()
        ...
        val fTxtColor = if (isFilterActive) filterEvColor else if (filterHov) 0xFFFFFFFF.toInt() else 0xFFAAAAAA.toInt()
    ...
    private fun drawVanillaButton(ctx: DrawContext, x: Int, y: Int, w: Int, h: Int, hovered: Boolean) {
        val fill    = if (hovered) 0xFF626262.toInt() else 0xFF484848.toInt()
        val hiLight = if (hovered) 0xFFCCCCCC.toInt() else 0xFFAAAAAA.toInt()
        val shadow  = 0xFF252525.toInt()
