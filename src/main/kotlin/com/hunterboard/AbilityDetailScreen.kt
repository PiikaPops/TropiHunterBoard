...
class AbilityDetailScreen(
    private val abilityTemplate: AbilityTemplate,
    private val parent: Screen
) : Screen(Text.literal("Ability Detail")) {
    ...
    override fun render(context: DrawContext, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        val abilityName: String = Text.translatable(abilityTemplate.displayName).string
        val nameX = panelX + (panelWidth - textRenderer.getWidth(abilityName)) / 2
        context.drawText(textRenderer, abilityName, nameX, panelTop + 6, 0xFFFFFFFF.toInt(), true)
        ...
        y += 12
        val description: String = Text.translatable(abilityTemplate.description).string
        if (description.isNotEmpty()) {
            ...
            context.drawText(textRenderer, "-", leftX + 4, y, 0xFF666666.toInt(), true)
            y += 10
        }
        y += 8
        ...
        val sectionLabel: String = Translations.tr("Pokémon with this ability")
        ...
        if (!AbilityData.reverseIndexReady) {
            ...
            val learners = AbilityData.getSpeciesWithAbility(abilityTemplate.name)
            if (learners.isEmpty()) {
                val noText: String = Translations.tr("No Pokémon has this ability")
                context.drawText(textRenderer, noText, leftX + 4, y, 0xFF666666.toInt(), true)
                y += 12
            } else {
                val grouped = learners.groupBy { it.isHidden }
                val groupOrder = listOf(false, true)
                val groupLabels = mapOf(
                    false to Translations.tr("Normal"),
                    true  to Translations.tr("Hidden")
                )

                for (isHidden in groupOrder) {
                    val group = grouped[isHidden] ?: continue
                    val label = groupLabels[isHidden] ?: ""
                    context.drawText(textRenderer, "$label:", leftX + 2, y, 0xFFAAAA55.toInt(), true)
                    y += 13

                    if (learnerViewMode == 0) {
                        y = renderIconGrid(context, mouseX, mouseY, group, leftX, y, contentW, contentTop, contentBottom)
                    } else {
                        y = renderList(context, mouseX, mouseY, group, leftX, y, contentW, contentTop, contentBottom)
                    }
                    y += 6
                }
            }
    ...
    /** Lookup species by resourceIdentifier.path — safe for names with spaces etc. */
