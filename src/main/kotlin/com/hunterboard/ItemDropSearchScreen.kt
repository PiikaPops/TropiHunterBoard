...
class ItemDropSearchScreen : Screen(Text.literal("Item Drop Search")) {
    ...
    private fun applySortAndFilter() {
        ...
        val searched = if (query.isEmpty()) base else base.filter { itemId ->
            ...
            displayName.lowercase().contains(query) || itemId.path.lowercase().contains(query)
        }
    ...
    override fun render(context: DrawContext, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        // Items tab (active)
        context.fill(itemsTabX, tY, itemsTabX + itemsTabW, tY + tabH, 0xFF2A2200.toInt())
        drawBorder(context, itemsTabX, tY, itemsTabW, tabH, ModConfig.accentColor())
        context.drawText(textRenderer, itemsLabel, itemsTabX + 4, tY + 2, ModConfig.accentColor(), true)
        ...
        if (!DropData.reverseIndexReady) {
            ...
            for (itemId in filteredItems) {
                if (y + rowHeight > resultsTop - rowHeight && y < resultsBottom + rowHeight) {
                    ...
                    val countText = "${count}\u2726"
                    context.drawText(textRenderer, countText, countColX, y + 5, 0xFF888888.toInt(), true)
                }
                y += rowHeight
            }
        }
