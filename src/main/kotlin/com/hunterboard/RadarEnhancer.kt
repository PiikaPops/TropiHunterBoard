...
object RadarEnhancer {
    ...
    /** Sends TropiMod's OpenRadarPayload (empty pool) to trigger a server-side radar scan. */
    fun triggerRadarOpen() {
        try {
            ...
            HunterBoard.LOGGER.info("[RadarEnhancer] Radar open triggered via keybinding")
        } catch (e: Exception) {
            HunterBoard.LOGGER.warn("[RadarEnhancer] triggerRadarOpen failed: ${e.message}")
        }
    }
    ...
    private fun drawSwitchToHBButton(ctx: DrawContext, mx: Int, my: Int) {
        ...
        ctx.fill(bx, by, bx + bw, by + 1, ModConfig.accentColor())
        ctx.fill(bx, by + SWITCH_BTN_H - 1, bx + bw, by + SWITCH_BTN_H, ModConfig.accentColor())
        ctx.fill(bx, by, bx + 1, by + SWITCH_BTN_H, ModConfig.accentColor())
        ctx.fill(bx + bw - 1, by, bx + bw, by + SWITCH_BTN_H, ModConfig.accentColor())
